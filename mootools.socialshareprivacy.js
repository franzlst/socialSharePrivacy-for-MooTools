/*
 * mootools.socialshareprivacy.js | 2 Klicks fuer mehr Datenschutz
 * Umschreibung des jQuery-Plugins socialSharePrivacy (Details siehe unten) von Franz Steinmetz.
 *
		...
		socialSharePrivacy: function (settings) {
			...
			if (facebook_on) {
				...
				var fb_code = new IFrame({ 
					src: 'http://www.facebook.com/plugins/like.php?locale=' + options.services.facebook.language + '&href=' + fb_enc_uri + '&send=false&amp;layout=button_count&width=120&show_faces=false&amp;action=' + options.services.facebook.action + '&colorscheme=light&font&height=21',
					scrolling: 'no',
					frameborder: 0,
					style: { border: 'none', overflow: 'hidden', width:145, height:21 },
			...
			if (twitter_on) {
				...
				var twitter_code = new IFrame({ allowtransparency: true, frameborder:0, scrolling: 'no', src:'http://platform.twitter.com/widgets/tweet_button.html?url=' + twitter_enc_uri + '&counturl=' + twitter_count_url + '&text=' + text + '&count=horizontal&lang=' + options.services.twitter.language, style: { width:130, height:25 }});
